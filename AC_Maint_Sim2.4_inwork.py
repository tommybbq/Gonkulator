 ...
 #               aircraft.onSchedule = False
    ...
    def Repair(self, aircraft, ld):
        ...
        with mech as req: #req the resource
            ...
            print(f"{Time.getTime(self.env)} - A/C {aircraft.tailNum} - {ld.workcenter} working mcn {id(ld)}.")
            ...
            if int(ld.timetoComplete) < Time.getMinLeftInShift(self.env):
                print(f"    shift time left {Time.getMinLeftInShift(self.env)} > time to complete {ld.timetoComplete}")
                yield self.env.timeout(ld.timetoComplete) #timeout
                setattr(ld, "timeWorked", ld.timetoComplete)
                setattr(ld, "complete", True)
                print(f"{Time.getTime(self.env)} - A/C {aircraft.tailNum} - {ld.workcenter} completed mcn {id(ld)}.")
                ...
                print(f"    shift time left {Time.getMinLeftInShift(self.env)} < repair time {ld.timetoComplete}")
