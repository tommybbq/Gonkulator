#To open this file use exec(open('AC_Maint_Sim3.1.txt').read())
"""
Created on Mon Jul 19 14:45:07 2021. Updated by Bryan Ek 9/16/2021
...
class g:
    ...
    """
#
#    gDistributions_float={"gaussian","uniform_float","exponential","beta"}#These are the currently implemented distributions.
#    gDistributions_int={"uniform_int","triangular","poisson","binomial"}
#    gDistributions_placeholders={"fixed","N/A"}
#    See https://docs.python.org/3/library/random.html, https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.binom.html, https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.poisson.html
#    for how currently implemented distributions want the parameters formatted.
#
    gVars = {}
    gVarDistributions={}#Strings of which distribution for each variable
    gVarParameter1={}
    gVarParameter2={}
    gVarMins= {}
    gVarMaxes= {}
...
#
    def getGVars():
        ...
        return g.gVars
#
    def setGVar(key,value):
        g.gVars[key]=value
#
    def readVarRanges():
        """
        Code to ingest input variable ranges
        Parameters
        ----------
        None
        Returns
        ...
        """
        """code to ingest input variables"""
        with open('inputDataRanges.csv', mode='r') as infile:
            reader = csv.reader(infile)
            for rows in reader:
                g.gVarMins[rows[0]] = rows[1]
                g.gVarMaxes[rows[0]] = rows[2]
                g.gVarStepsizes[rows[0]] = rows[3]
        g.gVarMins.pop('Variables')
        g.gVarMaxes.pop('Variables')
        g.gVarStepsizes.pop('Variables')
#
        """string to int"""
        for key in g.gVarMins:#It is probably best to keep them all the same type for each key. But apparently this leads to g.gVarMaxes and g.gVarStepsizes not parsing to int or float.
            try:
                g.gVarMins[key] = int(g.gVarMins[key])
            except ValueError:
                g.gVarMins[key] = float(g.gVarMins[key])
        for key in g.gVarMaxes:
            try:
                g.gVarMaxes[key] = int(g.gVarMaxes[key])
            except ValueError:
                g.gVarMaxes[key] = float(g.gVarMaxes[key])
        for key in g.gVarStepsizes:
            try:
                g.gVarStepsizes[key] = int(g.gVarStepsizes[key])
            except ValueError:
                g.gVarStepsizes[key] = float(g.gVarStepsizes[key])
        print(g.gVars)
#
    def readVarDistributions():
        """
        Code to ingest input variable distributions
        Parameters
        ----------
        None
        Returns
        ...
        """
        """code to ingest input variables"""
        with open('inputDataDistributions.csv', mode='r') as infile:
            reader = csv.reader(infile)
            for rows in reader:
                g.gVarDistributions[rows[0]] = rows[1]
                g.gVarParameter1[rows[0]] = rows[2]
                g.gVarParameter2[rows[0]] = rows[3]
        g.gVarDistributions.pop('Variables')
        g.gVarParameter1.pop('Variables')
        g.gVarParameter2.pop('Variables')
#
        """string to int"""
        for key in g.gVarParameter1:
            try:
                g.gVarParameter1[key] = int(g.gVarParameter1[key])
            except ValueError:
                g.gVarParameter1[key] = float(g.gVarParameter1[key])
        for key in g.gVarParameter2:
            try:
                g.gVarParameter2[key] = int(g.gVarParameter2[key])
            except ValueError:
                g.gVarParameter2[key] = float(g.gVarParameter2[key])
        print(g.gVars)
#
class Workcenter(Enum):
    """Maintainers' workcenters"""
    FL = auto()
    AF = auto()
    AVI = auto()
#
...
#   print()
#
#Updated to read ranges of inputs over which to simulate
#print("Reading input variable ranges")
#g.readVarRanges()
#for key in g.gVarMins:
#    g.gVars[key]=g.gVarMins[key]#Could I simply set g.gVars=g.gVarMins to avoid the for loop?
#rangeValues={key : numpy.arange(g.gVarMins[key],g.gVarMaxes[key]+g.gVarStepsizes[key],g.gVarStepsizes[key]) for key in g.gVarMins}#Ideally, split into int/float.
#keys=g.gVars.keys()
#for values in itertools.product(*rangeValues.values()):#numTrials should probably not be here. Will it cause a problem?
#    for iter_count,key in enumerate(keys):#This might not be the most efficient assignment method. Especially since likely only one value is updated at a time. But the simulation itself seems to take much more time. Can this be improved?
#    	g.gVars[key]=values[iter_count]
#    for run in range(g.gVars['numTrials']):
#        print("Run ", run+1, " of ", g.gVars['numTrials'], sep="")
#        my_sq_model = Squadron_Model(run)
#        my_sq_model.run()
#        print()
#    print("Finished all trials for the set of input values ",values, sep="")
#    print()
#
#Read in distribution values
print("Reading input variable distributions")
g.readVarDistributions()#Returns {}
import random
from scipy.stats import binom,poisson
for sample_num in range(1,g.gVarParameter1['numTotalSamples']+1):
    #Sample the values
    for key in g.gVarDistributions:
        if g.gVarDistributions[key]=='gaussian':
            g.gVars[key]=random.gauss(g.gVarParameter1[key],g.gVarParameter2[key])
        elif g.gVarDistributions[key]=='uniform_float':
            g.gVars[key]=random.uniform(g.gVarParameter1[key],g.gVarParameter2[key])
        elif g.gVarDistributions[key]=='exponential':
            g.gVars[key]=random.expovariate(g.gVarParameter1[key])
        elif g.gVarDistributions[key]=='beta':
            g.gVars[key]=random.betavariate(g.gVarParameter1[key],g.gVarParameter2[key])
        elif g.gVarDistributions[key]=='gamma':
            g.gVars[key]=random.gammavariate(g.gVarParameter1[key],g.gVarParameter2[key])
        elif g.gVarDistributions[key]=='uniform_int':
            print(g.gVarParameter1[key])
            print(g.gVarParameter2[key])
            g.gVars[key]=random.randint(g.gVarParameter1[key],g.gVarParameter2[key])
        elif g.gVarDistributions[key]=='triangular':
            g.gVars[key]=round(random.triangular(g.gVarParameter1[key],g.gVarParameter2[key]))
        elif g.gVarDistributions[key]=='poisson':
            g.gVars[key]=poisson.rvs(g.gVarParameter1[key],g.gVarParameter2[key])
        elif g.gVarDistributions[key]=='binomial':
            g.gVars[key]=binom.rvs(g.gVarParameter1[key],g.gVarParameter2[key])
        elif g.gVarDistributions[key]=='fixed':
            g.gVars[key]=g.gVarParameter1[key]
#        else it is not a value relevant to simulations. E.g. it is the number of trials or samples to be done.
    for trial_num in range(1, g.gVarParameter1['numTrials']+1):
        print("Trial ", trial_num, " of ", g.gVarParameter1['numTrials'], sep="")
        my_sq_model = Squadron_Model(trial_num)
        my_sq_model.run()
        print()
    print("Finished all trials for sample ", sample_num," of ", g.gVarParameter1['numTotalSamples'],sep="")
    print()
#
print("End Sim")